...
BUNDLE_PATH: Union[str, Path, None] = Path(
    "/Users/ryangrgurich/VS Code Local/corner-maze-encoder-pretrain/data/pt-files/corner-maze-render-base-iti-nocue-images-dataset.pt"
)

# Tolerances for duplicate detection (see _normalize_tolerance for scaling rules).
PER_CHANNEL_TOLERANCE: float = 20.0
...
OUTPUT_BUNDLE_PATH: Union[str, Path, None] = Path(
    f"/Users/ryangrgurich/VS Code Local/corner-maze-encoder-pretrain/data/pt-files/corner-maze-render-base-iti-nocue-images-duplicate-groups-{int(PER_CHANNEL_TOLERANCE)}-{int(MEAN_TOLERANCE)}.pt"
)

# Default path for saving duplicate group report (set to None to disable file output).
REPORT_PATH: Union[str, Path, None] = Path(
    f"/Users/ryangrgurich/VS Code Local/corner-maze-encoder-pretrain/data/reports/corner-maze-render-base-iti-nocue-images-duplicate-groups-{int(PER_CHANNEL_TOLERANCE)}-{int(MEAN_TOLERANCE)}.json"
)
...
def _build_singleton_summaries(
    ...
    def _catalog_descriptions(label_id: int) -> List[str]:
    ...
    for idx in singleton_indices:
        ...
        for desc in _catalog_descriptions(label_id):
...
def process_duplicates(
    ...
    singleton_summaries = _build_singleton_summaries(
        singleton_indices, label_ids, label_names, label_catalog
    )
                ...
                json.dump(
                    {
                        ...
                        "singleton_groups": singleton_summaries,
                    },
                    fh,
                    indent=2,
                )
