"""
Rotate config images while automatically updating start and cue directions.

Update the constants below:
    - `SOURCE_DIR`: folder containing the images for a single config (e.g. trl_e_n_xx)
    - `ROTATION_DEGREES`: rotation angle in degrees (must be a multiple of 45)
    - `CENTER`: board center for coordinate rotation (defaults to 6,6)

When run, the script:
    1. Computes the new start-arm and cue directions by rotating them the same
       amount as the coordinates (e.g. +90° turns east→south, north→east)
    2. Creates a sibling directory named after the rotated prefix
    3. Copies every PNG into the new directory, renaming each file so the
       prefix reflects the rotated start/cue and the coordinates reflect the
       rotated x,y pair.

Example:
    SOURCE_DIR       = Path("/path/to/trl_e_n_xx")
    ROTATION_DEGREES = 90

    => Output folder: /path/to/trl_s_e_xx
    => Filename:      trl_s_e_xx_<x_rot>_<y_rot>_<rest>.png
"""
...
# ========= USER PARAMETERS =========
SOURCE_DIR = Path("~/VS Code Local/corner-maze-encoder-pretrain/data/images/config-files-update/iti_w_x_sw")  # update to your source folder
ROTATION_DEGREES = 90                    # e.g., 90, 180, 270 (must be multiple of 45)
...
def rotate_direction_token(token: str, degrees: float) -> str:
    ...
    if token_norm not in _DIR_TO_INDEX:
        raise ValueError(f"Unsupported direction token '{token}'. Expected one of {_DIRECTION_ORDER}.")
...
def parse_filename(filename: str) -> Tuple[Tuple[str, ...], int, int, Tuple[str, ...], str]:
    """
    Parse filenames like 'trl_e_n_xx_1_1_ne_l.png'.

    Returns:
        prefix_parts   -> ('trl', 'e', 'n', 'xx')
...
def build_rotated_prefix(prefix_parts: Tuple[str, ...], rotation: float) -> str:
    """Rotate the start-arm and cue tokens inside the prefix."""
    if len(prefix_parts) < 3:
        raise ValueError(f"Prefix {prefix_parts!r} does not contain start/cue tokens.")
    updated = list(prefix_parts)
    updated[1] = rotate_direction_token(updated[1], rotation)
    updated[2] = rotate_direction_token(updated[2], rotation)
...
def process_directory(source_dir: Path, rotation: float) -> None:
    ...
    for file_path in sample_files:
        ...
        rotated_prefix = build_rotated_prefix(prefix_parts, rotation)
        ...
        new_name = f"{rotated_prefix}_{x_rot}_{y_rot}"
        if suffix_segment:
...
def main() -> None:
    if SOURCE_DIR == Path("~/path/to/trl_e_n_xx"):
