...
Process binocular PNG images into a single .pt dataset bundle.
...
where:
  ...
  - direction: 0–7 or compass (N, NE, E, SE, S, SW, W, NW)
  - eye: 'left' or 'right'
...
def main(image_dir: str, blur_radius: float, output_name: str):
    """
    Build and save a single .pt dataset from binocular PNGs.

    Args:
        image_dir (str): Path to directory containing images.
        blur_radius (float): Gaussian blur radius to apply.
        output_name (str): Output .pt filename.
    """
    root = Path(image_dir)
    if not root.exists():
    ...
    print(f"[DONE] Saved {out_path}  ->  x{tuple(X.shape)}, y{tuple(Y.shape)}")
    print(f"[DONE] Example labels: {labels[:3]}")


if __name__ == "__main__":
    # Example direct call (edit these for your run):
    # main("/path/to/images", 1.5, "binocular_dataset.pt")

    import sys
    if len(sys.argv) != 4:
        print("Usage: python build_binocular_bundle.py <image_dir> <blur_radius> <output_name.pt>")
        sys.exit(1)

    image_dir = sys.argv[1]
    blur_radius = float(sys.argv[2])
